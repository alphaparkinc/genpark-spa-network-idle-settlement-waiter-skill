# genpark-spa-network-idle-settlement-waiter-skill
...
**genpark-spa-network-idle-settlement-waiter-skill** is a deterministic, zero-dependency Python skill engineered for autonomous web browsing agents, self-healing DOM element selection, and bot defense evasion telemetry.

> **Executive Capability**: SPA network idle & DOM mutation settlement waiter (Cypress / Playwright)
...
result = client.await_dom_settlement()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-spa-network-idle-settlement-waiter-skill": {
      "command": "python",
      "args": ["/path/to/genpark-spa-network-idle-settlement-waiter-skill/mcp_server.py"]
    }
  }
}
```
